...
async function main(){
  ...
  async function expense(description:string, amount:number, category:string, paidById:string, splitIds:string[], date:string){
    const share=Number((amount/splitIds.length).toFixed(2));
    return prisma.expense.create({data:{groupId:group.id,title:description,amount,category,paidById,date:new Date(date),splits:{create:splitIds.map(userId=>({userId,amount:share}))}}});
  }
