...
export async function POST(req: Request) {
  ...
  const expense = await prisma.expense.create({ data: { groupId, title: description, amount, category, paidById, date: date ? new Date(date) : new Date(), splits: { create: splitUserIds.map(userId => ({ userId, amount })) } }, include: { splits: true, paidBy: true } });
